...
-- weighted search to weigh. The star map needed Dijkstra with a length tiebreak
-- because its lanes ran anywhere from 60 to 200 world units and the
-- geometrically shortest route could be one lane longer - and therefore a whole
...
-- `galaxy.adjacency` is sorted ascending when it is built (galaxy/graph.lua), so
...
function M.find(galaxy, from, to, max_hops)
