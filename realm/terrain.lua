...
--
-- Replaces realm/starclass.lua and keeps its shape deliberately, because
-- `realm/sim/tiles.lua` reads it the same way: a **ground** table and a
...
--
-- **A holding is exactly a tile with a feature.** The old generator also
-- promoted energetic tile classes, which meant a holding could exist with
...
--
-- Terrain is not rolled from a weight table like a tile class was. It is
...
-- city; the generator tops the count up to config.city_fraction afterwards,
...
-- hills, 12% mountains) these average 0.188, so `city_fraction` of 0.20 always
-- binds and every seed gets exactly the same number of cities - which is what
-- the old tile map did too, and what the economy sweep priced against. Letting
-- the roll win instead gave 28%, or 62 cities where the prices assume 44.
...
-- read today (tiles.yield prices gold by it); `science` is carried for the
...
-- holding - see the note above. `main/theme.lua` owns which glyph.
M.FEATURES = {
...
-- are entirely to the noise, which is the same trade realm/land.lua makes to
