...
-- client must generate byte-identical realms from the same seed, the fallback
...
if bitlib and not rawget(_G, "REALM_FORCE_PURE_BITOPS") then
...
function Rng:shuffle(list)
	for i = #list, 2, -1 do
		...
		-- Explicit temporary: see the note in realm/delaunay.lua. The swap
...
-- seed keeps producing the same tile positions even as the generator grows.
