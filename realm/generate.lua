...
local noise = require("realm.noise")
local graph = require("realm.graph")
local names_mod = require("realm.names")
local terrain_mod = require("realm.terrain")
local defaults = require("realm.config")
...
-- than the coast's own frequency (realm/land.lua) on purpose: the coastline
...
-- is what lets realm/terrain.lua's thresholds be read as the proportions they
...
--- Generate a map.
-- @param seed integer
-- @param cfg  optional overrides of realm/config.lua
function M.build(seed, cfg)
	...
	-- 1. The continent. Connected by construction - see realm/land.lua.
	...
	--
	-- The edge list is built only so the province carver has one: `graph.provinces`
	-- and `graph.province_adjacency` are written against index pairs and work
	...
	for i = 1, n do
		...
		for d = 1, #hex.DIRECTIONS do
			...
			-- Each edge once, with the lower index first: graph.provinces keys its
	...
	-- World positions. `pts` is what the province carver measures distance with,
	...
	-- 3. Carve into provinces. Unchanged from the tile map: balanced growth along
	--    adjacency, which keeps every province contiguous and comparable in size.
	local province_count = floor(n / cfg.tiles_per_province + 0.5)
	province_count = max(cfg.min_provinces, min(cfg.max_provinces, province_count))
	-- **The stream label is frozen at "regions".** A label is an arbitrary salt
	-- whose only job is to be stable: it derives which numbers this subsystem
	-- sees, so renaming it silently reshuffles every map ever generated. It was
	-- "regions" before the vocabulary rename and it stays "regions", which is
	-- what makes that rename provably behaviour-free. Same for "capitals" in
	-- realm/sim/state.lua. Change one only when you mean to change every seed.
	local owner, seeds = graph.provinces(rng.stream(seed, "regions"), pts, edges, adj, province_count)
	local radj = graph.province_adjacency(owner, edges, province_count)
	local palette = cfg.province_palette
	local colour_index = graph.colour_provinces(radj, #palette, province_count)

	-- 4. Name everything. Provinces first so the strongest vocabulary lands on the
	...
	--
	-- Two passes, because realm/terrain.lua thresholds on **ranks** rather than
	...
	for i = 1, n do
		...
		-- way with the threshold realm/terrain.lua uses, which is a cap.
	...
	--
	-- Unchanged in shape from the tile map: the count is topped up
	...
	local target = floor(n * (cfg.city_fraction or 0) + 0.5)
	local habitable_count = 0
	for i = 1, n do
		if tiles[i].habitable then habitable_count = habitable_count + 1 end
	end
	if habitable_count < target then
		local candidates = {}
		for i = 1, n do
			if not tiles[i].habitable then
				local entry = terrain_mod.by_id(tiles[i].terrain)
		...
		for i = 1, #candidates do
			...
			tiles[candidates[i].id].habitable = true
			promote = promote - 1
		end
	end
	...
	for i = 1, n do
		tiles[i].nx = tiles[i].x / half
		tiles[i].ny = tiles[i].y / half
	end
