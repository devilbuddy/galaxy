--- Deterministic name generation for tiles and provinces.
--
...
-- so no two tiles collide.
--
-- Changing anything here changes every seed's digest: names are hashed
-- (realm/digest.lua), so the documented determinism constants are re-baked
...
-- Grand nouns for province names: bigger in scale than a single tile's feature.
local PROVINCE_NOUNS = {
...
--- Generate one tile name via weighted templates.
function Namer:tile()
	local r = self.r
	for attempt = 1, 40 do
		local t = r:weighted({
		...
		-- so a large realm cannot stall on an exhausted template.
		if attempt > 12 then
...
--- Generate one province name. Rendered uppercase by the map, so kept in title
--- case here and left to the presentation layer.
function Namer:province()
	local r = self.r
	for attempt = 1, 40 do
		local t = r:weighted({ 30, 22, 18, 16, 14 })
		local name
		if t == 1 or t == 2 then
			local adj, noun = r:pick(PROVINCE_ADJECTIVES), r:pick(PROVINCE_NOUNS)
			-- Reject overlaps like "The Deep Deeps".
			if adj:lower():sub(1, 4) == noun:lower():sub(1, 4) then
				noun = r:pick(PROVINCE_NOUNS)
			end
			name = (t == 1 and "The " or "") .. adj .. " " .. noun
		elseif t == 3 then
			name = invent(r) .. " " .. r:pick(PROVINCE_NOUNS)
		elseif t == 4 then
			name = r:pick(PLACE_STEM) .. r:pick(PLACE_SUFFIX) .. " " .. r:pick(PROVINCE_NOUNS)
		else
			name = "The " .. r:pick(PROVINCE_NOUNS) .. " of " .. invent(r)
		end
...
--- A namer with its own uniqueness set. Province names are drawn before tile
--- names by convention, so the memorable vocabulary lands on provinces.
function M.new(r)
