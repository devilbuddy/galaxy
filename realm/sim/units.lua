--- The three things a city can put aboard a commander.
--
...
-- derived from the tile and whatever has been built on it, and whatever army
...
--   Interceptor   hunts ships. Three times itself against an army.
...
-- rename has to carry the old keys across or every commander in flight comes back
...
--
-- `against` is M.FORTIFICATION or M.ARMY. Anything else reads as army, which
-- is the safer default: a commander caught in the open is the case where being
-- wrong matters most.
function M.power(hold, against)
	local key = (against == M.FORTIFICATION) and "fortification" or "army"
	local total = 0
	for i = 1, #M.CATALOGUE do
...
-- Interceptor, then the Bombard - a battery is the last thing an army gives up.
--
