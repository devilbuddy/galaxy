...
-- The races in realm/sim/races.lua still declare growth, industry, research and
...
function M.of(player)
	...
	return {
		-- Movement. A commander's reach comes from their rank
		-- (realm/sim/commanders.lua); a race with a real mobility bonus adds a
		-- whole extra tile a turn. Anything finer would be arithmetic the
		...
		-- Intelligence. Extra tiles on top of the base.
		vision = e.vision or 0,

		-- War. `attack` weighs a commander's own strength; `defence` weighs how
		-- hard the tiles this player holds are to take off them - so it is
