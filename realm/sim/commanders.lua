--- Commanders: the named officer, and how they improve.
--
-- A commander is a named officer who moves on the map. The name and the face are
-- most of the point: a piece a player is attached to is worth more than a token,
-- and every player currently has exactly one.
--
-- Rank, reach and how much a commander can spend taking ground all derive from
...
-- player has to look up; "Admiral Kess" against "Commander Kess" is legible at a
-- glance, and it is the same information.
local RANKS = {
	{ from = 1,  title = "Commander" },
...
-- Surnames are invented rather than borrowed, for the same reason tile names
...
-- ones take a campaign. Experience is the **strength a commander has overcome**,
-- so a city is worth more than open country without any table having to say so,
...
-- can look at a route, count the tiles, and know which turn a commander lands.
function M.steps(commander, mods)
	local level = commander.level or 1
	local steps = rules.commander_steps
...
--
-- Where a commander starts, and where a broken one reforms. Rank raises it, which
-- is why a veteran is worth having before a single unit is loaded aboard.
function M.base_strength(commander, mods)
	local level = commander.level or 1
	local base = rules.commander_strength
...
function M.max_units(commander, mods)
	local level = commander.level or 1
	local units = rules.commander_units + rules.units_per_level * (level - 1)
...
--
-- `against` is `units.FORTIFICATION` or `units.ARMY`. The officer's own worth
...
--- The most strength a commander can hold: their own, plus a full complement.
--
-- **Rank does not cap what a commander carries.** It used to - the ceiling was
-- the rank base alone - and that walled the game shut: a fresh commander could
-- not cover a defended city, so could not win the battle that would have
-- promoted them, so never got any stronger. Every game stalled with two players
-- on three of the four provinces they needed and full purses they could not
-- spend.
--
...
function M.profile(commander, mods, race)
	...
	return {
		...
		army_power = M.power(commander, mods, units.ARMY),
