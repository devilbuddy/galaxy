--- Carving the map into contiguous provinces.
--
...
-- the substrate went from a pruned Delaunay tile network to a hex lattice.
...
-- rather than a graph decision - but it stays here because the province carver
-- below is written against `pts + edges + adj` and works on any graph, which is
-- exactly what let it survive the move off the tile map untouched.
--
...
-- a tile network. A lattice has no arbitrary connections to choose, so all three
-- went with the triangulation.
function M.adjacency(n, tiles)
	local adj = {}
	for i = 1, n do adj[i] = {} end
	for i = 1, #tiles do
		local a, b = tiles[i][1], tiles[i][2]
		adj[a][#adj[a] + 1] = b
		adj[b][#adj[b] + 1] = a
	end
...
--- Is every tile reachable from tile 1?
...
--- Choose `k` spread-out seed tiles, k-means++ style.
--
...
-- one enormous province. Sampling proportional to squared distance keeps the
...
--- Partition tiles into contiguous provinces of comparable size.
--
-- Plain multi-source Dijkstra produces wildly lopsided provinces: whichever seed
-- sits nearest the galactic core claims most of the map. So growth is balanced
-- instead - at every step the province that currently owns the fewest tiles takes
-- the cheapest tile on its own frontier. Expanding only along tiles keeps each
-- province contiguous in travel terms, and the balancing keeps them playable.
function M.provinces(r, pts, tiles, adj, k)
	local n = #pts
	...
	local cost = {}
	for i = 1, #tiles do
		local ia, ib = tiles[i][1], tiles[i][2]
	...
	-- One frontier per province, each a flat list of {tile, distance}. They stay
	...
	local claimed = 0
	local function claim(tile, province, dist)
		owner[tile] = province
		size[province] = size[province] + 1
		claimed = claimed + 1
		local nb = adj[tile]
		for i = 1, #nb do
			if owner[nb[i]] == 0 then
				local f = frontier[province]
				f[#f + 1] = { nb[i], dist + tile_cost(tile, nb[i]) }
			end
		end
	end
	...
	while claimed < n do
		-- Smallest province first; ties go to the lower index so the sweep order
		...
		-- Drop stale entries (tiles another province already took) by compacting
	...
	-- Only reachable if the tile graph is disconnected; fall back to geometry.
	for i = 1, n do
		if owner[i] == 0 then
...
--- Which provinces touch which, via tiles that cross a border.
function M.province_adjacency(owner, tiles, k)
	local adj = {}
	for i = 1, k do adj[i] = {} end
	for i = 1, #tiles do
		local ra, rb = owner[tiles[i][1]], owner[tiles[i][2]]
...
--- Greedy graph colouring so no two bordering provinces share a palette entry.
function M.colour_provinces(radj, palette_size, k)
	-- Colour the most-constrained provinces first: standard Welsh-Powell, which
