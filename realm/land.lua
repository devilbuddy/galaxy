...
-- spacing that got near a target tile count and accepted whatever it got; here
...
function M.build(seed, inner, target, opts)
	...
	while #land < target do
		...
		-- the end. Same shape as the province growth in realm/graph.lua.
		local w = 0
		for i = 1, #frontier do
			local e = frontier[i]
...
-- server produce the same array rather than merely the same set. `realm/wire.lua`
