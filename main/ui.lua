...
-- And a third, for the Noto emoji the interface draws (main/emoji.atlas,
-- generated by tools/import_emoji.py). Separate from the map's packed sheet
-- because a GUI node plays a whole named image and cannot take a UV rect.
M.EMOJI_ATLAS = "emoji"
M.FONT = "ui"
M.FONT_BOLD = "ui_bold"
...
--- A Noto emoji from the interface atlas, by the name main/theme.lua gave it.
--
-- **Never tinted.** A glyph is art rather than a shape, so the colour is white
-- times `alpha` - the same rule main/shaders/emoji.fp follows on the map, where
-- the vertex colour carries fog and nothing else. Fading one out is the only
-- thing a caller gets to do to it.
--
-- Falls back to nothing rather than erroring, like M.portrait: a missing glyph
-- must not take a screen down, and tools/test_wire.lua is what catches the
-- mismatch instead.
function M.emoji(x, y, size, name, alpha)
	local node = born(gui.new_box_node(vmath.vector3(x, y, 0),
		vmath.vector3(size, size, 0)))
	gui.set_pivot(node, gui.PIVOT_NW)
	gui.set_texture(node, M.EMOJI_ATLAS)
	gui.set_color(node, vmath.vector4(1, 1, 1, alpha or 1))
	if name then pcall(gui.play_flipbook, node, name) end
	return node
end
